...
// Create an ObservableList Object to hold the Associated Parts
	...
	public void addProductAddButtonListener(ActionEvent actionEvent) {
		...
		com.inventory.Part selectedPart = addProductTableView.getSelectionModel()
		                                                     .getSelectedItem();
	...
	public void removeAssociatedPartButtonListener(ActionEvent actionEvent) {
		...
		com.inventory.Part selectedPart = associatedPartTableView.getSelectionModel()
		                                                         .getSelectedItem();
		...
		if ( passCheck ) {
			try {
				...
				if ( productRowsAffected > 0 ) {
					...
					com.inventory.Product newProduct = new com.inventory.Product(randomId,
					                                                             newProductName,
					                                                             newProductPrice,
					                                                             newProductStock,
					                                                             newProductMin,
					                                                             newProductMax,
					                                                             associatedPartsList);
					com.inventory.Inventory.allProducts.add(newProduct);
	...
	public void initialize(URL url, ResourceBundle resourceBundle) {
		...
		addProductTableView.setItems(com.inventory.Inventory.allParts);
		
		// Wrap the Observable list in a FilteredList
		FilteredList<com.inventory.Part> filteredData = new FilteredList<>(com.inventory.Inventory.allParts, t -> true);
...
//                addProductErrorLabel.setText( "Error: No matching product found!" );
	...
	public void addProductsSearchFieldListener(ActionEvent actionEvent) {
		...
		com.inventory.Part selected = addProductTableView.getSelectionModel().getSelectedItem();
		if ( selected == null ) {
