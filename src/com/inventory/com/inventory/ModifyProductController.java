...
public class ModifyProductController implements Initializable {
	...
	@FXML
	private final ObservableList<com.inventory.Part> associatedPartsList = FXCollections.observableArrayList();
	@FXML
	...
	private void setupAvailablePartsTable() {
		...
		partStockCol.setCellValueFactory(cellData -> new SimpleIntegerProperty(cellData.getValue()
		                                                                               .getStock()).asObject());
		
		TableColumn<com.inventory.Part, Double> partPriceCol = new TableColumn<>("Price");
		partPriceCol.setCellValueFactory(cellData -> new SimpleDoubleProperty(cellData.getValue()
		                                                                              .getPrice()).asObject());
	...
	private void setupAssociatedPartsTable() {
		...
		assocPartIdCol.setCellValueFactory(cellData -> new SimpleIntegerProperty(cellData.getValue()
		                                                                                 .getId()).asObject());
		...
		assocPartStockCol.setCellValueFactory(cellData -> new SimpleIntegerProperty(cellData.getValue()
		                                                                                    .getStock()).asObject());
		
		TableColumn<com.inventory.Part, Double> assocPartPriceCol = new TableColumn<>("Price");
		assocPartPriceCol.setCellValueFactory(cellData -> new SimpleDoubleProperty(cellData.getValue()
		                                                                                   .getPrice()).asObject());
		
		modifyProductAssociatedPartsTableView.getColumns()
		                                     .addAll(assocPartIdCol,
		                                             assocPartNameCol,
		                                             assocPartStockCol,
		                                             assocPartPriceCol);
	}
				...
				try {
					...
					try ( PreparedStatement stmt = conn.prepareStatement(sql) ) {
						...
						if ( rowsAffected > 0 ) {
							...
							// Update UI list
							com.inventory.Product updatedProduct = new com.inventory.Product(id,
							                                                                 name,
							                                                                 price,
							                                                                 stock,
							                                                                 min,
							                                                                 max,
							                                                                 associatedPartsList);
							com.inventory.Inventory.updateProduct(com.inventory.Inventory.getSelectedProductIndex(),
							                                      updatedProduct);
